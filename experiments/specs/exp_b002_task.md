# Baseline Task: XGBoost (exp_b002)

Build a complete XGBoost training pipeline and get a CV RMSE score.
...
## Features to Engineer (same as LightGBM baseline for fair comparison)
...
If GPU fails (device="cuda" error), fall back to device="cpu".

Use early stopping: fit with eval_set=[(X_val, y_val)] and early_stopping_rounds=100.
...
for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_train, y_train, groups=well_ids)):
    ...
    fold_rmse = np.sqrt(np.mean((oof[val_idx] - y_train.iloc[val_idx])**2))
...
1. `experiments/oof/oof_xgb_exp_b002.npy` — shape (5092255,) float32
2. `experiments/test_preds/test_xgb_exp_b002.npy` — shape (19221,) float32
3. `experiments/results/exp_b002.json`:
```json
{
...
Write the script to `experiments/scripts/run_xgb_baseline.py` and run it.
