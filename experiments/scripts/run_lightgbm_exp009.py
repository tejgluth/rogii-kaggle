#!/usr/bin/env python3
"""LightGBM exp009 with Typewell geology features at DTW-aligned TVT."""
...
OOF_PATH = OOF_DIR / "oof_lightgbm_exp009.npy"
TEST_PREDS_PATH = TEST_PREDS_DIR / "test_lightgbm_exp009.npy"
RESULT_PATH = RESULTS_DIR / "exp009.json"
        ...
        [
            {
                ...
                "description": "Typewell Geology labels and layer geometry at exp006 DTW-aligned TVT.",
                "notes": result["notes"],
...
def main() -> None:
    ...
    delta = exp006_rmse - cv_rmse
    notes = (
        f"Built on exp006 DTW lag features and added Typewell Geology at aligned TVT, "
        f"native categorical geology/adjacent-layer labels, fold-safe smoothed geology "
        f"target encoding, and containing-layer thickness plus distance-to-top/bottom. "
        f"Compared with exp006 RMSE {exp006_rmse:.4f}, exp009 "
        f"{'improved' if delta > 0 else 'did not improve'} by {abs(delta):.4f} RMSE. "
        f"The signal should help when layer labels provide direct TVT context beyond GR; "
        f"it can hurt if DTW picks the wrong local alignment or if broad labels are too coarse."
    )
