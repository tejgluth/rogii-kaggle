#!/usr/bin/env python3
"""CatBoost exp016 on the exp011 deeper Typewell geology feature matrix."""
...
from src.features import (  # noqa: E402
    ...
    build_exp011_feature_matrix,
)
from src.postprocess import postprocess_per_well  # noqa: E402


EXPERIMENT_ID = "exp016"
BASE_EXPERIMENT = "exp011"
EXP011_RMSE = 34.838539123535156
...
OOF_PATH = OOF_DIR / "oof_catboost_exp016.npy"
TEST_PREDS_PATH = TEST_PREDS_DIR / "test_catboost_exp016.npy"
RESULT_PATH = RESULTS_DIR / "exp016.json"

TARGET_ENCODING_COLUMN = "tw_geology_target_enc"
FEATURES = [feature for feature in EXP011_FEATURES if feature != TARGET_ENCODING_COLUMN]
NUMERIC_FEATURES = [
    feature for feature in FEATURES if feature not in EXP011_GEOLOGY_CATEGORICAL_FEATURES
]
...
def read_csv(path: Path) -> pd.DataFrame:
    if GPU and gpu_pd is not None:
        return gpu_pd.read_csv(path).to_pandas()
...
def make_pool(x: pd.DataFrame, y: np.ndarray | None = None) -> Pool:
    return Pool(data=x, label=y, cat_features=EXP011_GEOLOGY_CATEGORICAL_FEATURES)
        ...
        [
            {
                ...
                "description": "CatBoost trained on exp011 deeper Typewell geology features.",
                "notes": result["notes"],
...
def main() -> None:
    ...
    train_df, global_gr_median, global_tvt_input_median = build_exp011_feature_matrix(
        train_df,
        train_typewell,
    )
    test_df, _, _ = build_exp011_feature_matrix(
        test_df,
        test_typewell,
        global_gr_median,
    ...
    print(f"Dropped target-encoded geology column: {TARGET_ENCODING_COLUMN}")
    print(f"Native CatBoost categorical features: {EXP011_GEOLOGY_CATEGORICAL_FEATURES}")
    ...
    delta_vs_exp011 = cv_rmse - EXP011_RMSE
    delta_vs_exp014 = cv_rmse - EXP014_RMSE
    notes = (
        f"Trained CatBoost on the exp011 deeper-geology feature set and dropped "
        f"{TARGET_ENCODING_COLUMN} so all aligned and offset geology labels are handled "
        f"as native CatBoost categorical features. Compared with exp011 LightGBM RMSE "
        f"{EXP011_RMSE:.4f}, exp016 "
        f"{'improved by' if delta_vs_exp011 < 0 else 'trailed by'} {abs(delta_vs_exp011):.4f} "
        f"RMSE; compared with exp014 CatBoost RMSE {EXP014_RMSE:.4f}, it "
        f"{'improved by' if delta_vs_exp014 < 0 else 'trailed by'} {abs(delta_vs_exp014):.4f}. "
        f"The deeper features help if CatBoost can use ordered layer context without "
        f"overfitting DTW alignment noise; otherwise they add noisy stratigraphic detail."
    )
    ...
    result = {
        ...
        "features_added": EXP011_DEEP_GEOLOGY_CATEGORICAL_FEATURES + EXP011_DEEP_GEOLOGY_NUMERIC_FEATURES,
        "dropped_features": [TARGET_ENCODING_COLUMN],
        "categorical_features": EXP011_GEOLOGY_CATEGORICAL_FEATURES,
