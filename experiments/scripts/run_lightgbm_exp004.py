#!/usr/bin/env python3
"""LightGBM exp004: baseline plus expanded GR statistics."""
...
from src.features import add_exp004_gr_features  # noqa: E402
...
DESCRIPTION = (
    "Expanded GR rolling statistics and per-well normalization: multi-window "
    "rolling mean/std/min/max/slope, GR diffs, GR percentile/z-score, and "
    "well-level GR aggregates."
)

REPO_ROOT = Path(__file__).resolve().parents[2]
OOF_PATH = REPO_ROOT / "experiments" / "oof" / "oof_lightgbm_exp004.npy"
TEST_PREDS_PATH = REPO_ROOT / "experiments" / "test_preds" / "test_lightgbm_exp004.npy"
RESULT_PATH = REPO_ROOT / "experiments" / "results" / "exp004.json"
...
FEATURES = BASE_FEATURES + EXP004_FEATURES
...
def write_result(
    ...
    notes = (
        ...
        f"exp004 {helped}. These cheap GR context features summarize local "
        "texture and per-well normalization, but they do not explicitly align "
        "lateral GR to the typewell, so any gain is expected to be smaller "
        "than correlation/DTW features."
    )
    ...
    result = {
        ...
        "features_added": EXP004_FEATURES,
