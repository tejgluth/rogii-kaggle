#!/usr/bin/env python3
"""LightGBM exp006 with Typewell GR DTW lag features."""
...
from src.features import add_dtw_lag_features  # noqa: E402
...
OOF_PATH = OOF_DIR / "oof_lightgbm_exp006.npy"
TEST_PREDS_PATH = TEST_PREDS_DIR / "test_lightgbm_exp006.npy"
RESULT_PATH = RESULTS_DIR / "exp006.json"
...
FEATURES = BASE_FEATURES + DTW_FEATURES
        ...
        [
            {
                ...
                "description": "Typewell GR constrained DTW lag features over lag range [-40, 40].",
                "notes": result["notes"],
...
def main() -> None:
    ...
    notes = (
        f"Added constrained DTW Typewell GR lag features over lags [-40, 40] "
        f"with step 2, window 41, Sakoe-Chiba radius 5, computed every 5th row "
        f"and interpolated back to all rows. Compared with exp_b001 RMSE "
        f"{baseline_rmse:.4f}, exp006 {'improved' if helped else 'did not improve'} "
        f"by {abs(improvement):.4f} RMSE. DTW should help where local GR motifs align "
        f"with stretched or squeezed typewell patterns; it may fail when TVT_input is "
        f"sparse/noisy or GR motifs are non-unique across nearby lags."
    )
