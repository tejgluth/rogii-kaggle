#!/usr/bin/env python3
"""LightGBM exp007 combining Typewell xcorr and DTW lag features."""
...
from src.features import (  # noqa: E402
    ...
    add_typewell_correlation_features,
)
...
OOF_PATH = OOF_DIR / "oof_lightgbm_exp007.npy"
TEST_PREDS_PATH = TEST_PREDS_DIR / "test_lightgbm_exp007.npy"
RESULT_PATH = RESULTS_DIR / "exp007.json"
...
INTERACTION_FEATURES = [
    "xcorr_minus_dtw_lag",
    "lag_agreement_strength",
]
FEATURES = BASE_FEATURES + TYPEWELL_FEATURES + DTW_FEATURES + INTERACTION_FEATURES
        ...
        [
            {
                ...
                "description": "Combined Typewell GR cross-correlation and DTW lag alignment features.",
                "notes": result["notes"],
...
def main() -> None:
    ...
    improvement_vs_base = baseline_rmse - cv_rmse
    improvement_vs_exp006 = exp006_rmse - cv_rmse
    helped_vs_exp006 = improvement_vs_exp006 > 0
    notes = (
        f"Combined exp003 Typewell GR rolling cross-correlation features and exp006 constrained DTW "
        f"features on top of exp_b001, plus xcorr_minus_dtw_lag and lag_agreement_strength. "
        f"Compared with exp_b001 RMSE {baseline_rmse:.4f}, exp007 changed RMSE by "
        f"{improvement_vs_base:.4f}; compared with exp006 target {exp006_rmse:.4f}, it "
        f"{'improved' if helped_vs_exp006 else 'did not improve'} by {abs(improvement_vs_exp006):.4f}. "
        f"The combined signals should help when linear correlation and stretch-aware DTW agree on the "
        f"same typewell offset; disagreement can add noise where GR motifs are ambiguous or repeated."
    )
