#!/usr/bin/env python3
"""XGBoost exp010 on the exact exp006 DTW feature matrix."""
...
from src.features import EXP006_FEATURES, build_exp006_feature_matrix  # noqa: E402


EXPERIMENT_ID = "exp010"
BASE_EXPERIMENT = "exp006"
...
OOF_PATH = OOF_DIR / "oof_xgboost_exp010.npy"
TEST_PREDS_PATH = TEST_PREDS_DIR / "test_xgboost_exp010.npy"
RESULT_PATH = RESULTS_DIR / "exp010.json"
        ...
        [
            {
                ...
                "description": (
                    "XGBoost trained on the exact exp006 constrained DTW feature matrix."
                ),
                "notes": result["notes"],
...
def main() -> None:
    ...
    train_df, global_gr_median, global_tvt_input_median = build_exp006_feature_matrix(
        train_df,
        train_typewell,
    )
    test_df, _, _ = build_exp006_feature_matrix(
        test_df,
        test_typewell,
        global_gr_median,
    ...
    delta_vs_exp006 = cv_rmse - EXP006_RMSE
    competitive = abs(delta_vs_exp006) <= 1.0
    notes = (
        f"Trained XGBoost on the exact exp006 constrained DTW feature matrix with no "
        f"additional features. Compared with exp006 LightGBM RMSE {EXP006_RMSE:.4f}, "
        f"exp010 {'improved by' if delta_vs_exp006 < 0 else 'trailed by'} "
        f"{abs(delta_vs_exp006):.4f} RMSE. The feature set tests algorithmic diversity; "
        f"the saved OOF predictions are useful for stacking "
        f"{'because the model is competitive' if competitive else 'but the model is outside the ~1 RMSE competitive band'}."
    )
    ...
    result = {
        ...
        "features_used": EXP006_FEATURES,
        "n_features": len(EXP006_FEATURES),
        "training_time_seconds": float(training_time),
        "notes": notes,
        "oof_path": str(OOF_PATH.relative_to(REPO_ROOT)),
        "test_path": str(TEST_PREDS_PATH.relative_to(REPO_ROOT)),
        "feature_params": {
            "source_feature_set": "exp006",
            "max_lag": 40,
            "lag_step": 2,
            "window": 41,
