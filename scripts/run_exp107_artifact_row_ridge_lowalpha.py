"""exp107: lower-alpha refinement after exp106 selected the lower grid edge."""
...
RESULT = ROOT / "experiments/results/exp107.json"
SUBMISSION = ROOT / "submissions/exp107_artifact_row_ridge_lowalpha_clean_oof.csv"

ALPHAS = [100.0, 200.0, 500.0, 750.0, 1_000.0, 1_500.0]
WELL_WEIGHTS = np.arange(0.50, 0.55 + 1e-12, 0.00625)
ROW_WEIGHTS = np.arange(-0.125, -0.075 + 1e-12, 0.0025)
...
def main() -> None:
    ...
    if int(sub["tvt"].isna().sum()):
        raise ValueError("missing clean low-alpha row ridge predictions")
    ...
    payload = {
        ...
        "phase": "artifact_row_ridge_lowalpha",
        "selection_rule": (
            "exp105/106 structure with lower ridge alphas after exp106 selected the lower grid edge; "
