"""exp109: very-low-alpha probe around exp108's clean OOF optimum."""
...
RESULT = ROOT / "experiments/results/exp109.json"
SUBMISSION = ROOT / "submissions/exp109_artifact_row_ridge_verylowalpha_clean_oof.csv"

ALPHAS = [0.1, 1.0, 2.0, 5.0, 10.0, 15.0]
WELL_WEIGHTS = np.arange(0.50625, 0.54375 + 1e-12, 0.00625)
ROW_WEIGHTS = np.arange(-0.165, -0.115 + 1e-12, 0.005)
...
def main() -> None:
    ...
    if int(sub["tvt"].isna().sum()):
        raise ValueError("missing clean very-low-alpha row ridge predictions")
    ...
    payload = {
        ...
        "phase": "artifact_row_ridge_verylowalpha",
        "selection_rule": (
            "exp108 structure with very low ridge alphas around the exp108 non-heldout OOF optimum; "
