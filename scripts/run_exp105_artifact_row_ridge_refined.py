"""exp105: fine-grid refinement of exp104 row-ridge plus well correction.

This reuses exp104's clean structure and only refines the ridge alpha and
well/row residual weights around the non-heldout OOF optimum found by exp104.
The heldout/sample wells remain excluded from every fit and selection step.
"""
...
RESULT = ROOT / "experiments/results/exp105.json"
SUBMISSION = ROOT / "submissions/exp105_artifact_row_ridge_refined_clean_oof.csv"

ALPHAS = [3_000.0, 10_000.0, 30_000.0, 100_000.0, 300_000.0, 1_000_000.0]
WELL_WEIGHTS = np.linspace(0.40, 0.60, 9)
ROW_WEIGHTS = np.array([-0.18, -0.16, -0.14, -0.12, -0.10, -0.08, -0.06, -0.04, -0.02, 0.0], dtype=float)
...
def main() -> None:
    ...
    if int(sub["tvt"].isna().sum()):
        raise ValueError("missing clean refined row ridge predictions")
    SUBMISSION.parent.mkdir(parents=True, exist_ok=True)
    sub.to_csv(SUBMISSION, index=False)

    clean_holdout_rmse = rmse(clean_pred, y[heldout])
    payload = {
        "experiment_id": "exp105",
        "phase": "artifact_row_ridge_refined",
        "selection_rule": (
            "exp104 structure with a refined alpha and residual-weight grid around the exp104 non-heldout OOF optimum; "
