"""exp106: narrow fine-grid refinement of exp105.

This keeps exp105's model family fixed and only searches a narrow alpha/weight
grid around the exp105 non-heldout OOF optimum.  The sample-submission wells are
held out from every fit and selection step; they are used once for audit.
"""
...
RESULT = ROOT / "experiments/results/exp106.json"
SUBMISSION = ROOT / "submissions/exp106_artifact_row_ridge_finegrid_clean_oof.csv"

ALPHAS = [1_000.0, 2_000.0, 3_000.0, 5_000.0, 8_000.0, 10_000.0]
WELL_WEIGHTS = np.arange(0.475, 0.575 + 1e-12, 0.0125)
ROW_WEIGHTS = np.arange(-0.14, -0.06 + 1e-12, 0.005)
...
def main() -> None:
    ...
    if int(sub["tvt"].isna().sum()):
        raise ValueError("missing clean fine-grid row ridge predictions")
    SUBMISSION.parent.mkdir(parents=True, exist_ok=True)
    sub.to_csv(SUBMISSION, index=False)

    clean_holdout_rmse = rmse(clean_pred, y[heldout])
    payload = {
        "experiment_id": "exp106",
        "phase": "artifact_row_ridge_finegrid",
        "selection_rule": (
            "exp105 structure with a narrow alpha and residual-weight grid around the exp105 non-heldout OOF optimum; "
