"""exp085: second-stage LGBM residual correction after exp084."""
import json
...
def main():
    ...
    base_oof = np.load(ROOT / "experiments/oof/oof_stack_exp084.npy")
    base_test = np.load(ROOT / "experiments/test_preds/test_stack_exp084.npy")
    residual = (y - base_oof).astype(np.float32)
    base_score = rmse(base_oof[is_lat], y[is_lat])
    print(f"base exp084 rmse={base_score:.6f} mse={base_score ** 2:.6f}", flush=True)
    ...
    np.save(ROOT / "experiments/oof/oof_stack_exp085.npy", pred.astype(np.float32))
    np.save(ROOT / "experiments/test_preds/test_stack_exp085.npy", test.astype(np.float32))
    np.save(ROOT / "experiments/oof/oof_lgbm_exp085_resid.npy", oof_resid)
    np.save(ROOT / "experiments/test_preds/test_lgbm_exp085_resid.npy", test_resid)
    payload = {
        "experiment_id": "exp085",
        "phase": "residual",
        "base": "exp084",
    ...
    (ROOT / "experiments/results/exp085.json").write_text(json.dumps(payload, indent=2))
