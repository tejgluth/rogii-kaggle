"""exp110: full artifact-feature row ridge plus exp100 well correction.

This is a side branch from exp109.  Instead of the compact 52-feature row ridge,
it fits a ridge residual model on all artifact feature columns.  Alpha and the
well/row residual weights are selected only on non-heldout OOF rows.
"""
...
from run_exp104_artifact_row_ridge_plus_well import gbr_d4, make_well_oof, ridge


RESULT = ROOT / "experiments/results/exp110.json"
SUBMISSION = ROOT / "submissions/exp110_artifact_fullfeature_ridge_plus_well_clean_oof.csv"

ALPHAS = [100.0, 1_000.0, 10_000.0, 100_000.0, 1_000_000.0]
WELL_WEIGHTS = np.arange(0.45, 0.60 + 1e-12, 0.025)
ROW_WEIGHTS = np.arange(-0.25, 0.25 + 1e-12, 0.025)
...
def main() -> None:
    ...
    if best_selection is None:
        raise RuntimeError("no full-feature ridge selection was produced")
    ...
    if int(sub["tvt"].isna().sum()):
        raise ValueError("missing clean full-feature ridge predictions")
    SUBMISSION.parent.mkdir(parents=True, exist_ok=True)
    sub.to_csv(SUBMISSION, index=False)

    clean_holdout_rmse = rmse(clean_pred, y[heldout])
    payload = {
        "experiment_id": "exp110",
        "phase": "artifact_fullfeature_ridge_plus_well",
        "selection_rule": (
            "artifact-only equal OOF base; exp100 gbr_d4 well correction OOF and full artifact-feature ridge residual OOF "
            "are combined by weights selected on non-heldout rows only; row OOF uses GroupKFold by non-heldout wells; "
            "final models train on non-heldout data and are audited once on heldout wells"
        ),
        ...
        "row_feature_count": int(len(feature_cols)),
        "alphas": ALPHAS,
