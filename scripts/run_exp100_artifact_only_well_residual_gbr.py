"""exp100: artifact-only well residual with compact dev-selected model grid.

exp099 showed that per-well residual offsets improve the clean artifact-only
base.  A wider non-heldout CV sweep found gradient boosting on well summaries to
be stronger than the shallow RF.  This script keeps a compact reproducible grid
of the dev-leading families, selects model/weight on non-heldout wells only, and
audits the three sample-submission wells once.
"""
...
RESULT = ROOT / "experiments/results/exp100.json"
SUBMISSION = ROOT / "submissions/exp100_artifact_only_well_residual_gbr_clean_oof.csv"
...
def main() -> None:
    ...
    if int(sub["tvt"].isna().sum()):
        raise ValueError("missing clean artifact-only GBR well residual predictions")
    ...
    payload = {
        ...
        "phase": "artifact_only_well_level_residual_gbr",
        "selection_rule": (
            "artifact-only base; compact model grid and residual weight selected by 5-fold CV over non-heldout wells only; "
