"""exp028: Port of the public 9.956-LB ROGII kernel.

Adapted from `nihilisticneuralnet/9-251-rogii-wellbore-geology-prediction-dwt-based`
and `romantamrazov/rogii-better-solution-lb-9-956` notebooks. Targets <10 RMSE.

Key differences from original:
- Removes /kaggle/input artifacts dependency — runs from raw data only.
- Saves OOF + test predictions in original CSV row order (length 5092255 train, 19221 test)
  for compatibility with our stacking pipeline (instead of just lateral rows).
- Skips per-iteration savgol smoothing of test by default; can be applied as postproc.
- Adapted device flags for our Linux ARM64 + CUDA setup.

Run: .venv/bin/python scripts/run_exp028_full_kernel.py
"""
...
os.makedirs(ROOT/".numba",exist_ok=True)
print("ok")


# ==================== Cell 2 ====================
from pathlib import Path as _PathDummy
...
OUT=ROOT/"submissions/exp028_full_kernel.csv"
OUT.parent.mkdir(parents=True, exist_ok=True)
EXP_OOF = ROOT / "experiments/oof/oof_combined_exp028.npy"
EXP_TEST = ROOT / "experiments/test_preds/test_combined_exp028.npy"
EXP_RESULT = ROOT / "experiments/results/exp028.json"
...
LGB_BASE=dict(
    ...
    verbose=-1, n_jobs=-1, device_type="cpu", max_bin=255,
)
LGB_CONFIGS=[
...
CB_P=dict(
    ...
    loss_function="RMSE", random_seed=42, task_type="GPU",
...
# === Save OOF / test predictions in original full-CSV row order for stacking ===
import json as _json
# Reload absolute TVT predictions per lateral row → broadcast to full CSV layout
# train: pred_abs = base + final_oof   (for lateral rows only; the kernel only produced features for lateral)
train_oof_full = np.full(5092255, np.nan, np.float32)
test_pred_full = np.full(19221, np.nan, np.float32)
# build map from (well,id) → CSV row index
def _csv_row_index_map(directory):
    idx_map = {}
    cursor = 0
    for p in sorted(directory.glob('*__horizontal_well.csv')):
        ...
        for i in range(n):
            idx_map[(wid, i)] = cursor + i
        cursor += n
    return idx_map, cursor

train_idx_map, n_train_rows = _csv_row_index_map(TRAIN_DIR)
test_idx_map,  n_test_rows  = _csv_row_index_map(TEST_DIR)

# Absolute predictions (lateral-only) → full arrays
train_pred_abs = base + final_oof
for (wid, rowid), pred in zip(
        zip(train_df['well'].values, train_df['id'].values),
        train_pred_abs):
    # id format = "{wid}_{rowindex}"
    rid = int(rowid.split('_')[-1])
    if (wid, rid) in train_idx_map:
        train_oof_full[train_idx_map[(wid, rid)]] = pred
# Fill heel (non-lateral) rows with their known TVT_input
# Reload each train CSV's TVT to fill heel rows
cursor = 0
import pandas as _pd
for p in sorted(TRAIN_DIR.glob('*__horizontal_well.csv')):
    df_ = _pd.read_csv(p, usecols=['TVT', 'TVT_input'])
    nrow = len(df_)
    mask = df_['TVT_input'].notna().values
    train_oof_full[cursor:cursor+nrow][mask] = df_['TVT'].values[mask].astype(np.float32)
    cursor += nrow

# Test: final smoothed prediction
test_pred_abs = test_df2['pred'].values.astype(np.float32)
for (wid, rowid), pred in zip(
        zip(test_df2['well'].values, test_df2['id'].values),
        test_pred_abs):
    rid = int(rowid.split('_')[-1])
    if (wid, rid) in test_idx_map:
        test_pred_full[test_idx_map[(wid, rid)]] = pred
# Fill heel rows in test with their known TVT_input
cursor = 0
for p in sorted(TEST_DIR.glob('*__horizontal_well.csv')):
    df_ = _pd.read_csv(p, usecols=['TVT_input'])
    nrow = len(df_)
    mask = df_['TVT_input'].notna().values
    test_pred_full[cursor:cursor+nrow][mask] = df_['TVT_input'].values[mask].astype(np.float32)
    cursor += nrow

# Fill any remaining NaN with a safe default
train_oof_full = np.where(np.isnan(train_oof_full), float(_pd.Series(train_oof_full).median()), train_oof_full)
test_pred_full = np.where(np.isnan(test_pred_full), float(_pd.Series(test_pred_full).median()), test_pred_full)

np.save(EXP_OOF, train_oof_full.astype(np.float32))
np.save(EXP_TEST, test_pred_full.astype(np.float32))
_json.dump({
    "experiment_id": "exp028", "model": "kernel_port_lgb3+cb1+ridge+pp",
    "phase": "feature_engineering",
    "cv_rmse_lateral_delta": float(min(r_avg, r_stk)),
    "cv_rmse_abs_tvt": float(best_r),
    "alpha": ALPHA, "tau": TAU, "w_pf": W_PF,
    "n_features": len(feature_cols),
    "notes": "Port of public 9.956-LB kernel. Particle filter ANCC+Z, 7 beam searches, multi-scale NCC, formation b_well anchoring, KNN imputation. Saved OOF/test in full CSV row order for stacking.",
    "oof_path": str(EXP_OOF.relative_to(ROOT)),
