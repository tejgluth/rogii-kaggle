"""exp098: well-level residual correction selected on non-heldout wells.

The exp095 row-level OOF blend still has well-to-well bias.  This experiment
aggregates artifact features per well, learns one residual offset per well, and
selects the correction model/weight with 5-fold CV over non-heldout wells.  The
sample-submission wells are audited only after selection.
"""
...
RESULT = ROOT / "experiments/results/exp098.json"
SUBMISSION = ROOT / "submissions/exp098_well_residual_rf_clean_oof.csv"
...
def main() -> None:
    ...
    if int(sub["tvt"].isna().sum()):
        raise ValueError("missing clean well residual predictions")
    ...
    payload = {
        "experiment_id": "exp098",
        "phase": "well_level_artifact_residual",
        "selection_rule": (
            "model family and residual weight selected by 5-fold CV over non-heldout wells only; "
        ...
        "note": "The target is the per-well mean residual y_abs - exp095_base. Heldout labels are not used for fitting or selection.",
    }
