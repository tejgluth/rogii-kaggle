# ROGII Clean Artifact Row Ridge Very-Low-Alpha Submission
...
- trains a conservative row-level ridge residual with GroupKFold over
  non-sample wells;
- selects the very-low-alpha ridge setting and residual weights using
  non-sample OOF rows only;
- writes `/kaggle/working/submission.csv`.

Local clean-holdout audit for this input-only variant was RMSE `6.8267` / MSE
`46.6040`, improving exp105's RMSE `6.8401` and exp100's RMSE `6.8816`.
